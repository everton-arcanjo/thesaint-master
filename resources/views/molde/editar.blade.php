...
<div class="pd-ltr-20 customscroll customscroll-10-p height-100-p xs-pd-20-10 mCustomScrollbar _mCS_3 mCS-autoHide" style="position: relative; overflow: visible;"><div id="mCSB_3" class="mCustomScrollBox mCS-minimal-dark mCSB_vertical mCSB_outside" tabindex="0" style="max-height: none;"><div id="mCSB_3_container" class="mCSB_container" style="position: relative; top: 0px; left: 0px;" dir="ltr">
    ...
    <div class="min-height-200px">
        <div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
            ...
            <form role="form" name="formMolde" action="/molde/update/{{ $objMolde->mol_id }}" method="post">
                @csrf

                <div class="form-group">
                    <label>Molde</label> <span style="color: red">*</span>
                    <input type="text" class="form-control" maxlength="30" placeholder="Molde" name="molde" id="moldeMolde" value="{{ $objMolde->mol_molde }}" required="required">
                </div>

                <div class="form-group">
                    <label>Unidade</label> <span style="color: red">*</span>
                     <select name="unidade" class="form-control" id="unidade" required="required">
                        <option @if( $objMolde->mol_unidade == 'MT' ) selected="selected" @endif value="MT">Metros</option>
                        <option @if( $objMolde->mol_unidade == 'GR' ) selected="selected" @endif value="GR">Gramas</option>
                        <option @if( $objMolde->mol_unidade == 'KG' ) selected="selected" @endif value="KG">Kilo</option>
                     </select>
                </div>

                <div class="form-group">
                    <label>Consumo</label> <span style="color: red">*</span>
                    <input type="text" class="form-control" placeholder="Consumo" name="consumo" id="moldeConsumo" value="{{ $objMolde->mol_consumo }}" required="required">
                </div>
...
@section('javascript')
    <script type="javascript" src="{{asset('js/molde.js')}}"></script>
@endsection
